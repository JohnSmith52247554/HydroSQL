...
void testParse()
{
    ...
    std::string str3 = "SELECT (index, name, age) FROM test\n\
    WHERE age >= 30\n\
    ORDER BY index ASC;";
    ...
    // auto list = Parser::tokenize(expr);
    // auto lt = Parser::parseExpr(list.begin(), --list.end());

    auto list = Parser::tokenize(str);
    auto aff = Parser::parse(list);
    std::string result;
    if (aff != nullptr)
        aff->execute(true, result);
    std::cout << result << std::endl;

    list = Parser::tokenize(str2);
    aff = Parser::parse(list);
    if (aff != nullptr)
        aff->execute(true, result);
    std::cout << result << std::endl;

    list = Parser::tokenize(str3);
    aff = Parser::parse(list);
    if (aff != nullptr)
        aff->execute(true, result);
    std::cout << result << std::endl;

    list = Parser::tokenize(str4);
    aff = Parser::parse(list);
    if (aff != nullptr)
        aff->execute(true, result);
    std::cout << result << std::endl;

    list = Parser::tokenize(select_all);
    aff = Parser::parse(list);
    if (aff != nullptr)
        aff->execute(true, result);
    std::cout << result << std::endl;

    list = Parser::tokenize(str5);
    aff = Parser::parse(list);
    if (aff != nullptr)
        aff->execute(true, result);
    std::cout << result << std::endl;
